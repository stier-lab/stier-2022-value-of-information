...
est.NPV<-function(years,K,A,r,phi.CV.low,phi.CV.high,delta,process.noise,p,B.start,B.lim,B.crit,max.F,phi.CV.seed,process.noise.seed,c){
  ...
  for (i in 1:years){
    ...
    #Simplified based on reviewer 1's comments about hockey stick being too liberal and wanting harvest to stop below Bmsy 
    if (Bhat.vec[i]>Bmsy) F<- max.F
    if (Bhat.vec[i]<=Bmsy) F<- 0#Fo+b*Bhat.vec[i]
    ...
    #the error in thh second year is a funciton of the new biomass not the old 
...
source(here("code","ModelParameters_v1.R"))# load base parameters

mf0.5<-max.F*0.5
mf1.3<-max.F*1.3
mf1.9<-max.F*1.9
phi.CV.low=phi.CV.high=0.5 #high precision cv = 0
A=10
B.start=100


t<-est.NPV(years,K,A,r,phi.CV.low,phi.CV.high,delta,process.noise=0.5,p,B.start,B.lim,B.crit,max.F=Fmsy,phi.CV.seed,process.noise.seed,c)
print(t)
...
plot(t$Bhat,type="l",col=1)
lines(t$B,type="l",col=2)
...
value = repeat.model2(n.iters,B.start=70,B.lim,years,K,A,r,phi.CV,delta=.05,process.noise=0,p,max.F=mf0.5,phi.seeds,process.seeds)
